...
Hacker News のトップには毎日良質な記事が並ぶが、英語を読み解きながら重要度を判断するのは思ったより認知コストがかかる。「重要な記事だけ、日本語で、勝手にまとめてくれないか」——そんな動機から作ったのが **tech-digest** だ。

**完成物**: https://hayatotoyoda.github.io/tech-digest/

毎朝 07:00 JST に GitHub Actions が自動実行し、Hacker News のトップ記事を Claude AI が日本語でまとめ、GitHub Pages に公開する。サーバーレス・ゼロ運用コストで動いている。
